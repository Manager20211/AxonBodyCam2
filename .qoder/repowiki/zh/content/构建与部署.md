...
- 构建脚本（build.gradle）
  - 插件：应用Fabric Loom与Maven发布插件，启用Remap任务链
...
- 插件与版本
  - 应用Fabric Loom与Maven发布插件，版本来自属性文件
...
- 触发条件
  - 对PR与push事件自动触发
- 运行环境
  - Ubuntu 24.04，使用Microsoft JDK 25
- 步骤
  - 检出仓库
  - 校验Gradle Wrapper
  - 设置JDK并赋予可执行权限
...
GH->>Act : 触发workflow
Act->>Act : checkout仓库
Act->>GW : wrapper-validation
Act->>Act : setup-java(25)
Act->>Act : chmod +x ./gradlew
Act->>GW : ./gradlew build
GW->>Build : 执行构建任务
Build-->>Art : 生成JAR等产物
Act->>Art : upload-artifact(build/libs)
```
...
- CI构建失败
  ...
  - 排查：确认JDK版本、Gradle Wrapper校验通过、可执行权限已赋予
...
## 结论
本指南系统梳理了BodyCam项目的构建与部署配置，从Gradle脚本、属性文件、设置与包装器，到CI自动化流程与产物管理。遵循本文建议，可在本地与CI环境中高效完成构建、测试与打包，并为后续发布与版本管理打下坚实基础。
...
- 发布说明
  - 在GitHub Releases中编写变更摘要、已知问题与迁移指引
  - 参考：[README.md](file://README.md)
- 发布配置
